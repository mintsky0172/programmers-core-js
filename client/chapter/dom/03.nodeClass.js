...
first.innerHTML += '-HOLA!'; // setter

// - textContent
// * 요소 내의 텍스트에 접근
// * 태그는 제외하고 오로지 텍스트만 추출

first.textContent = '쉬는시간 끝';
...
// 클로저가 무엇인가요?(면접질문)
// 음... 함수가 데이터를 기억하지 못할 것이란 건 면접관님도 잘 알고 계시죠?
// 그렇다면, 오늘날 React의 hook은 어떻게 데이터를 저장할까요?
// 그건 바로 함수는 데이터를 기억하지 못하지만 자신이 살던 환경은 기억할 수 있기 때문입니다.
// 안쪽 함수는 바깥쪽 함수에 대한 모든 참조가 가능합니다.
// 이는 함수가 바깥 환경을 들여다볼 수 있는 outer env ref를 가지고 있기 때문이죠?
// 그래서 함수를 내보낸다면 그걸 이어받은 함수는 환경을 기억하기 때문에 GC 수집대상이 되지 않습니다.
// 이를 이용해 변수를 저장하는 트릭이죠.
